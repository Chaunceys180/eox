...
///
/// The [Slidable] widget calls the [onDismissed] callback either after its size
/// has collapsed to zero (if [resizeDuration] is non-null) or immediately after
...
/// See also:
///
///  * [SlideToDismissDrawerDelegate], which creates slide actions that are
///  displayed like drawers
...
abstract class SlideToDismissDelegate {
  ...
  /// Called when the widget changes size (i.e., when
  /// contracting before being dismissed).
  final VoidCallback onResize;

  /// The amount of time the widget will spend
  /// contracting before [onDismissed] is called.
  ///
  ...
  ///
  /// If non-zero value is given then widget moves in
  /// cross direction depending on whether
                  ...
                  builder: (context, child) {
                    return new Stack(
                      children: List.generate(ctx.state.actionCount, (index) {
                        // For the main actions we have to reverse the order if
                        // we want the last item at the bottom of the stack.
...
class SlideActionBuilderDelegate extends SlideActionDelegate {
  ...
  /// builder callback.
  ///
  /// The [builder] must not be null. The [actionCount]
  /// argument must not be positive.
...
/// A handle to various properties useful
/// while calling [SlidableDelegate.buildActions].
///
/// See also:
///
///  * [SlidableState], which create this object.
///  * [SlidableDelegate] and other delegates inheriting it,
///  which uses this object in [SlidableDelegate.buildActions].
...
///  * [SlidableBehindDelegate], which creates slide
///  actions that stay behind the item
///  while it's sliding.
///  * [SlidableScrollDelegate], which creates slide
///  actions that follow the item
///  while it's sliding.
///  * [SlidableDrawerDelegate], which creates slide actions that
///  are displayed like drawers
///  while the item is sliding.
abstract class SlidableDelegate {
  ...
  /// The threshold used to know if a movement was fast and
  /// request to open/close the actions.
...
/// A delegate that creates slide actions which stretch
/// while the item is sliding.
...
/// A delegate that creates slide actions which stay behind
...
/// A delegate that creates slide actions which follow
/// the item while it's sliding.
...
/// A delegate that creates slide actions which animate
/// like drawers while the item is sliding.
            ...
            builder: (context, child) {
              return new Stack(
                children: List.generate(count, (index) {
                  // For the main actions we have to reverse the order if we
                  // want the last item at the bottom of the stack.
...
///
/// If the direction is [Axis.horizontal], this widget can
/// be slid to the left or to the right,
...
class Slidable extends StatefulWidget {
  ...
  ///
  /// The [actions] contains the slide actions that appears when the child
  /// has been dragged down or to the right.
  /// The [secondaryActions] contains the slide actions that appears when
  /// the child has been dragged up or to the left.
  ///
  /// The [delegate] and [closeOnScroll] arguments must not be null.
  /// The [actionExtentRatio]
  /// and [showAllActionsThreshold] arguments must be greater or equal
  /// than 0 and less or equal than 1.
  ///
  ...
  /// Creates a widget that can be slid.
  ///
  /// The [actionDelegate] is a delegate that builds the slide actions
  /// that appears when the child has been dragged down or to the right.
  /// The [secondaryActionDelegate] is a delegate that builds the slide
  /// actions that appears when the child has been dragged up or to the left.
  ///
  /// The [delegate], [closeOnScroll] and [enabled] arguments must not
  /// be null. The [actionExtentRatio]
  /// and [showAllActionsThreshold] arguments must be greater or equal
  /// than 0 and less or equal than 1.
  ///
  /// The [key] argument must not be null if the `slideToDismissDelegate`
  ...
  /// A delegate that builds slide actions that appears when the
  ...
  /// The offset threshold the item has to be dragged in
  /// order to show all actions
  /// in the slide direction.
  ///
  /// Represented as a fraction, e.g. if it is 0.4 (the default), then the item
  /// has to be dragged at least 40% of the slide actions extent towards
  /// one direction to show all actions.
  final double showAllActionsThreshold;

  /// Defines the duration for card to go to final position or to come back to
  /// original position if threshold not reached.
  final Duration movementDuration;

  /// Specifies to close this slidable after the
  /// closest [Scrollable]'s position changed.
  ///
  /// Defaults to true.
          ...
          assert(() {
            if (_resizeAnimation.status != AnimationStatus.forward) {
              ...
              throw new FlutterError(
                  ...
                  'Make sure to implement the onDismissed handler '
                      'and to immediately remove the Slidable\n'
